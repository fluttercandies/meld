...
Use the `Build a transition` panel to choose the start and end independently, including mixed-format transitions. `Path data` uses raw SVG `d` strings, `SVG markup` exercises the portable element parser and viewBox handling, and `Font glyphs` converts Flutter's `IconData` references into geometry through caller-owned font bytes. The swap action, progress scrubber, spring playback, and reverse-to-start action work across all three input families.
