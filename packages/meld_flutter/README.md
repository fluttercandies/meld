...
The widget supports declarative, controlled-progress, and command-driven modes; `MeldIconTheme` supplies shared paint tokens; `MeldPaintStyle` supports outline, fill, and fill-plus-outline rendering; and `MeldMotionMode.user` follows Flutter reduced-motion settings. Add a `label` for semantics or set `excludeFromSemantics` for decorative icons.
